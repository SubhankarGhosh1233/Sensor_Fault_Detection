...
class Model_pusher:
    ...
    model_trainer_artifact:ModelTrainerArtifact):
        try:
            ...
            self.model_resolver=ModelResolver(model_registry=self.model_pusher_config.save_model_dir)#*1
    ...
    def  initiate_model_pusher(self,)->ModelPusherArtifact:
        try:
            ...
            #before  saving save model dir(saving some_where)
            logging.info(f"Saving model into save model dir")
            transformer_path=self.model_resolver.get_latest_save_transformer_path()
            model_path=self.model_resolver.get_latest_save_model_path()
            target_encoder_path=self.model_resolver.get_latest_save_target_encoder_path()
            #save model dir(saving same time)
            save_object(file_path=transformer_path,obj=transformer)
            save_object(file_path=model_path,obj=model)
            save_object(file_path=target_encoder_path,obj=target_encoder)

            model_pusher_artifact = ModelPusherArtifact(puher_model_dirL=self.model_pusher_config.puher_model_dir,
